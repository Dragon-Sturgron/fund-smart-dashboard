...
export async function onRequestGet(context) {
  ...
  const settled = await Promise.allSettled(codes.map(code => getHistoryData(code)));
  const data = [];
  const errors = [];
  ...
  return json({ ok: true, data, errors, mode: 'parallel-batch' }, 200, errors.length ? 'public, max-age=20' : 'public, max-age=900, stale-while-revalidate=86400');
}
